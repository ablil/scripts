...
def main(
    target=None, port=None, listen=False, command=False, upload=None, execute=list()
):
    """ main function"""
    if target:
...
if __name__ == "__main__":
    ...
    subgroup1.add_argument(
        "-t", "--target", nargs=1, type=str, metavar="ip", help="target ip"
    )
    ...
    listen_mode.add_argument(
        "-c", "--command", action="store_true", help="command mode"
    )
    listen_mode.add_argument(
        "-e",
        "--execute",
        nargs=1,
        metavar="file",
        help="execute cmd from files and send output",
    )
    listen_mode.add_argument(
        "-u",
        "--upload",
        nargs=1,
        metavar="dst",
        help="receivce data and store it in dst",
    )
